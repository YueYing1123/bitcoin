...
ROUND = "round-13-20260518-official-ds-gpt-f1"
...
DS_JSONL = OUTDIR / "ds_official_outputs.jsonl"
GPT_JSONL = OUTDIR / "gpt55_gold_outputs.jsonl"
    ...
    for attempt in range(max_retries):
        ...
        try:
            response = client.with_options(timeout=timeout).responses.create(
                model=model,
                instructions="You are a legal document structured extraction assistant. Output strict JSON only.",
                input=prompt + "\n\nOutput JSON.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    provider_audits = [
        ...
        run_provider("gpt", sample_rows=sample_rows, texts=texts, prompt_template=prompt_template, args=args),
    ]
